...
# generates a random pid, checks if its within range, checks if its already in use 
...
print("PIDs in use before calling allocate_pid(): %s \n" %(PIDS_IN_USE))
...
# calls release_pid(15):
# adding 15 to the array just for the sake of testing release_pid(), given allocate_pid() is random
print("\nPIDs in use after calling allocate_pid() 5 times: %s" %(PIDS_IN_USE))
print ("\nadding 15 for testing...")
PIDS_IN_USE.append(15) 
print("\nIDs before calling release_pid(15): %s" %(PIDS_IN_USE))
release_pid(15)
print("PIDS_IN_USE after calling release_pid(15): %s" %(PIDS_IN_USE))
